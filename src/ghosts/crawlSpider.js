...
export const createCrawlSpider = ({ onRevealPeak, onNeedFleeTarget, onDespawn, seed = 1 }) => {
  ...
  const applyLegJitter = (intensity) => {
    legs.forEach((leg, i) => {
      leg.rotation.x = Math.sin(elapsed * 22 + i * 1.3) * 0.3 * intensity
    })
  }
  ...
  const update = (delta, cameraPosition) => {
    ...
    if (state === 'capturableLinger') {
      ...
      // bounded so it never wanders out of frame on its own.
      skitterTimer -= delta
      if (skitterTimer <= 0) {
