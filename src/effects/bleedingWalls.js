...
const buildDecalSlot = (seed, audio) => {
  const { texture, material, update, isComplete } = createBleedingWallTexture({ seed })
  const mesh = new THREE.Mesh(new THREE.PlaneGeometry(DECAL_WIDTH, DECAL_HEIGHT), material)
  mesh.visible = false
  ...
  const placeAt = (point) => {
    ...
    if (audio) audio.playPositionalOneShot('wallDrip', point.position, { volume: 0.7, refDistance: 0.6, maxDistance: 5 })
  }
...
export const createBleedingWalls = ({ surfaceSampler, audio, getIntensity }) => {
  const slots = []
  for (let i = 0; i < POOL_SIZE; i++) slots.push(buildDecalSlot(1000 + i, audio))
