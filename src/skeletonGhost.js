...
const LEAN_MAX = 0.16 // meters it creeps forward off its anchor (wall/furniture spawns only)
...
// Floor-spawned entities walk toward the player instead of leaning off a
// wall anchor — real translation across the floor, legs alternating in
// sync with movement speed, not just in-place jitter.
const WALK_SPEED = 0.55 // meters per second
const WALK_STOP_DISTANCE = 1.1 // meters from camera it stops at and starts staring instead
const WALK_CYCLE_HZ = 1.3 // full stride cycles per second at WALK_SPEED
const WALK_LEG_SWING = 0.55 // radians, hip swing amplitude
const WALK_KNEE_BEND = 0.5 // radians, knee bend amplitude (in phase with the trailing part of each stride)
const WALK_ARM_SWING = 0.35 // radians — arms swing opposite their same-side leg (contralateral gait)
const WALK_SPINE_SWAY = 0.06
...
export const createSkeletonGhostInstance = (template, { onRevealPeak, onNeedFleeTarget, onDespawn, onFootstep }) => {
  ...
  // Rest rotation for each bone, captured right after cloning (before any
  // pose is applied) — the rig does NOT rest at 0,0,0 per bone (that's only
  // true for bones with no baked orientation, which this FBX-sourced rig
  // doesn't have). Every pose offset below is applied relative to this,
  // never as an absolute value, or the bone snaps away from its natural
  // joint angle entirely — which is what was producing the contorted,
  // spaghetti-limbed look instead of a bent crawl pose.
  const restRotation = (bone) => (bone ? { x: bone.rotation.x, y: bone.rotation.y, z: bone.rotation.z } : null)
  const restSpine01 = restRotation(spine01)
  const restSpine02 = restRotation(spine02)
  const restHead = restRotation(headBone)
  const restArmL = restRotation(armL)
  const restArmR = restRotation(armR)
  const restThighL = restRotation(thighL)
  const restThighR = restRotation(thighR)
  const restCalfL = restRotation(calfL)
  const restCalfR = restRotation(calfR)
  ...
  let isRefind = false
  let pointType = 'wall'
  let walkPhase = 0 // advances only while actually walking, not while stopped/staring
  let lastFootstepHalfCycle = 0
  ...
  const spawnAt = (point, surfaceNormal, opts = {}) => {
    ...
    pointType = opts.pointType || 'wall'
    walkPhase = 0
    lastFootstepHalfCycle = 0
  ...
  const applyCrawlPose = (intensity) => {
    ...
    if (spine01) spine01.rotation.x = restSpine01.x + 0.55 * intensity
    if (spine02) spine02.rotation.x = restSpine02.x + 0.45 * intensity
    if (headBone) headBone.rotation.x = restHead.x + 0.3 * intensity

    // Arms bent forward/down, like dragging the body along the surface.
    if (armL) armL.rotation.x = restArmL.x - 1.1 * intensity
    if (armR) armR.rotation.x = restArmR.x - 1.1 * intensity
    if (armL) armL.rotation.z = restArmL.z + 0.25 * intensity
    if (armR) armR.rotation.z = restArmR.z - 0.25 * intensity

    // Legs bent at hip and knee — a standing T/A-pose leg reads as "just
    // standing there"; a bent one reads as "climbing/crawling out."
    if (thighL) thighL.rotation.x = restThighL.x + 0.9 * intensity
    if (thighR) thighR.rotation.x = restThighR.x + 0.7 * intensity
    if (calfL) calfL.rotation.x = restCalfL.x - 1.3 * intensity
    if (calfR) calfR.rotation.x = restCalfR.x - 1.0 * intensity
  }

  // Real alternating gait for floor-spawned entities actually walking
  // across open floor, as opposed to applyCrawlPose/applyTwitch's
  // hunched-and-jittering-in-place look for wall/furniture spawns.
  // `strideIntensity` fades the stride out as it slows to a stop, so it
  // settles rather than snapping straight to standing-still.
  const applyWalkCycle = (phase, strideIntensity) => {
    const swing = Math.sin(phase)
    const swingOpp = Math.sin(phase + Math.PI)
    // Knee bends more while that leg is trailing/lifting (roughly the back
    // half of its swing), not symmetrically — reads as an actual footstep
    // rather than a leg swinging like a pendulum.
    const kneeL = Math.max(0, Math.sin(phase - Math.PI / 2))
    const kneeR = Math.max(0, Math.sin(phase + Math.PI / 2 - Math.PI))

    if (thighL) thighL.rotation.x = restThighL.x + swing * WALK_LEG_SWING * strideIntensity
    if (thighR) thighR.rotation.x = restThighR.x + swingOpp * WALK_LEG_SWING * strideIntensity
    if (calfL) calfL.rotation.x = restCalfL.x - kneeL * WALK_KNEE_BEND * strideIntensity
    if (calfR) calfR.rotation.x = restCalfR.x - kneeR * WALK_KNEE_BEND * strideIntensity

    // Contralateral arm swing (opposite arm to same-side leg) — the
    // standard human/humanoid walk gait, and what stops a walk cycle from
    // reading as a limp.
    if (armL) armL.rotation.x = restArmL.x + swingOpp * WALK_ARM_SWING * strideIntensity
    if (armR) armR.rotation.x = restArmR.x + swing * WALK_ARM_SWING * strideIntensity

    if (spine01) spine01.rotation.z = restSpine01.z + swing * WALK_SPINE_SWAY * strideIntensity
    if (headBone) headBone.rotation.x = restHead.x + 0.12 * strideIntensity
  }
  ...
  const applyTwitch = (intensity) => {
    ...
    // All of these are absolute (rest + offset), never `+=` — accumulating
    // onto a bone every frame with no reset (the previous spine02.z bug)
    // drifts unbounded over time, especially for re-find entities that can
    // now stay revealed for many seconds with no auto-timeout.
    if (spine02) spine02.rotation.z = restSpine02.z + Math.sin(elapsed * 14) * 0.14 * intensity
    if (headBone) headBone.rotation.y = restHead.y + Math.sin(elapsed * 9 + 1) * 0.3 * intensity
    if (armL) armL.rotation.x = restArmL.x - 1.1 * Math.min(1, intensity + 0.3) + Math.sin(elapsed * 11) * 0.25 * intensity
    if (armR) armR.rotation.x = restArmR.x - 1.1 * Math.min(1, intensity + 0.3) + Math.cos(elapsed * 12) * 0.22 * intensity
    if (thighL) thighL.rotation.z = restThighL.z + Math.sin(elapsed * 7) * 0.15 * intensity
    if (thighR) thighR.rotation.z = restThighR.z + Math.cos(elapsed * 7.5) * 0.15 * intensity
  }
  ...
  const update = (delta, cameraPosition) => {
    ...
    if (state === 'capturableLinger') {
      if (pointType === 'floor' && cameraPosition) {
        // Walk across the floor toward the player, real footstep-synced
        // gait — distinct from the wall/furniture lean-forward below,
        // which never actually translates far from its anchor point.
        ...
        if (dist > WALK_STOP_DISTANCE) {
          toCamera.normalize()
          const step = Math.min(WALK_SPEED * delta, dist - WALK_STOP_DISTANCE)
          root.position.addScaledVector(toCamera, step)
          faceCamera(cameraPosition)
          walkPhase += delta * WALK_CYCLE_HZ * Math.PI * 2
          // Fade the stride in/out over the last bit of the approach so it
          // settles into the stare rather than snapping to a dead stop.
          const strideFade = Math.min(1, (dist - WALK_STOP_DISTANCE) / 0.4 + 0.15)
          applyWalkCycle(walkPhase, strideFade)

          // One footstep cue per foot plant — each half of the stride
          // cycle is one foot touching down — not a continuous loop.
          const halfCycle = Math.floor(walkPhase / Math.PI)
          if (halfCycle > lastFootstepHalfCycle) {
            lastFootstepHalfCycle = halfCycle
            if (onFootstep) onFootstep(root.position)
          }
        } else {
          // Arrived — behaves exactly like the wall/furniture "closing in"
          // stare from here: escalating twitch, no more movement.
          faceCamera(cameraPosition)
